...
def manual_sweeping(keithley, ser, pixel_number, sweep_count, start, stop, step, delay, username, area, direction, radiation, output_folder):
    ...
    - radiation (float): Radiation intensity for efficiency calculations.
    - output_folder (str): Folder where output files are saved.
    ...
    for i in range(sweep_count):
        ...
        # Save data for the specific pixel for this sweep in a unique text file
        sweep_file_path = os.path.join(output_folder, f'{username}_pixel_{pixel_number}_sweep_{i+1}.txt')
        with open(sweep_file_path, 'w') as sweep_file:
            sweep_file.write("Voltage (mV), Current (mA), Time (s)\n")
            for v, c, t in zip(voltage_values, current_values, time_values):
                sweep_file.write(f"{v:.6f}, {c:.6f}, {t:.6f}\n")
...
def mppt(keithley, ser, pixel_number, Pmax, Pnom, Isc, Voc, Imax, Vmax, FF, efficiency, voltage_change, end_voltage_differ, runtime_minutes, delay_mS, username, radiation, area, output_folder):
    ...
    with open(text_file_path, 'w') as log_file:
        ...
        while True:
            ...
            start_voltage = (Vmax + end_voltage_differ) / 1000.0    
            ...
            if start_voltage>end_voltage:
                ...
                log_file.write(f"{elapsed_time_minutes:.2f}, {Pmax:.2f}, {Vmax:.2f}, {Imax:.2f}, {FF:.3f}, {efficiency*100:.2f}\n")
            else:
